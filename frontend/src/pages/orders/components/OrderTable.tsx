        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-slate-100 bg-slate-50">
              ...
              <th className="px-5 py-4 text-xs font-semibold uppercase tracking-wider text-slate-500">Nhan vien</th>
              <th className="px-5 py-4 text-right text-xs font-semibold uppercase tracking-wider text-slate-500">Tong tien</th>
              <th className="px-5 py-4 text-center text-xs font-semibold uppercase tracking-wider text-slate-500">Trang thai</th>
              <th className="px-5 py-4 text-center text-xs font-semibold uppercase tracking-wider text-slate-500">Thanh toan</th>
            ...
            {orders.length === 0 ? (
              <tr>
                <td colSpan={8} className="px-5 py-16 text-center">
                  ...
                  <div className="font-medium text-slate-600">Chua co hoa don phu hop</div>
                  <div className="mt-1 text-sm text-slate-400">Thu doi bo loc hoac tao hoa don tu man hinh POS.</div>
                </td>
              </tr>
            ) : (
              orders.map((order) => (
                <tr key={order.id} className="hover:bg-slate-50/70">
                  ...
                  <td className="px-5 py-4">
                    <div className="text-sm font-medium text-slate-800">{order.customers?.name || 'Khach le'}</div>
                  ...
                  <td className="px-5 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => onView(order)} className="rounded-md p-1.5 text-blue-600 hover:bg-blue-50" title="Xem chi tiet">
                        <HiOutlineEye className="h-5 w-5" />
                      </button>
                      <button onClick={() => onDownloadPdf(order)} className="rounded-md p-1.5 text-slate-600 hover:bg-slate-100" title="Tai PDF">
                        <HiOutlineDownload className="h-5 w-5" />
                      </button>
                      {canCancel && order.status !== 'cancelled' && (
                        <button onClick={() => onCancel(order)} className="rounded-md p-1.5 text-red-600 hover:bg-red-50" title="Huy hoa don">
